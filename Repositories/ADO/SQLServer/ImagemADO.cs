...
namespace GWI.Repositories.ADO.SQLServer
{
    public class ImagemADO
    {
        ...
        public void add(Models.Imagens imagens)
        {
            using (SqlConnection connection = new SqlConnection(this.connectionString))
            {
                ...
                using (SqlCommand command = new SqlCommand())
                {
                    ...
                    command.CommandText = "insert into imagens (nt_titulo, nt_subtitulo, nt_texto, nt_data_publicação, nt_ativo, nt_cat_id, nt_sct_id, nt_p_id) values (@Baldan e Marchesan abrem falência,@url da imagem ,@,@valor); select convert(int,@@identity) as id;;";
        ...
        {
            List<Models.Imagens> imagens = new List<Models.Imagens  >();
            ...
            using (SqlConnection connection = new SqlConnection(this.connectionString))
            {
                ...
                using (SqlCommand command = new SqlCommand())
                {
                    ...
                    while (dr.Read())
                    {
                        Models.Imagens imagem = new Models.Imagens();
